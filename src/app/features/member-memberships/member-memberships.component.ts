...
export class MemberMembershipsComponent implements OnInit {
  member: MemberDto | null = null; memberships: MembershipDto[] = []; lessonReservations: ReservationDto[] = []; packages: StudioPackageDto[] = []; selectedPackageId: number | null = null; assigning = false; packageDrawerVisible = false;
  private memberId = 0;
  constructor(private readonly route: ActivatedRoute, private readonly router: Router, private readonly memberService: MemberService, private readonly membershipService: MembershipService, private readonly packageService: StudioPackageService, private readonly reservationService: ReservationService, private readonly cdr: ChangeDetectorRef) {}
  ...
  onTabChange(tab: string | number | undefined): void { if (tab === 'memberships') this.load(); if (tab === 'lessons') this.loadLessons(); }
